...
def verify_merged_dataframe(merged_df, original_df, dataset_name, file_path):
    if len(merged_df) != len(original_df):
        logging.error(f"The number of rows in the Merged {dataset_name} DataFrame does not match the original.")
        raise ValueError(f"The number of rows in the Merged {dataset_name} DataFrame does not match the original.")
    else:
        logging.info(f"The number of rows in the Merged {dataset_name} DataFrame matches the original.")
        merged_df.to_csv(file_path, index=False)
        logging.info(f"Merged {dataset_name} DataFrame successfully saved to {file_path}\n")
