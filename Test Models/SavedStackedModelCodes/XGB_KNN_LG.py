...
def conduct_optuna_study(X_train, y_train):
    ...
    def knn_objective(trial):
        try:
            ...
            n_neighbors_knn = trial.suggest_int('n_neighbors_knn', 1, 50)  # Number of neighbors
            weights_knn = trial.suggest_categorical('weights_knn', ['uniform', 'distance'])  # Weight function
            algorithm_knn = trial.suggest_categorical('algorithm_knn', ['auto', 'ball_tree', 'kd_tree', 'brute'])  # Algorithm to compute the nearest neighbors
            p_knn = trial.suggest_int('p_knn', 1, 2)  # Power parameter for the Minkowski distance (1 for Manhattan, 2 for Euclidean)

            # Handling class imbalance (binary classification case)
            scale_pos_weight_knn = len(y_train[y_train == 0]) / len(y_train[y_train == 1])
...
def main():
    ...
    try:
        ...
        for fold_idx, (X_train, X_test, y_train, y_test) in enumerate(folds, start=1):
            ...
            model_path = os.path.join(base_dir, 'Test Models', 'Stacked Models (Optuna)', f'XGB_KNN_LG_Fold_{fold_idx}.pkl')
            params_path = os.path.join(base_dir, 'Test Models', 'Stacked Models (Optuna)', f'XGB_KNN_LG_Best_Params_Fold_{fold_idx}.json')
