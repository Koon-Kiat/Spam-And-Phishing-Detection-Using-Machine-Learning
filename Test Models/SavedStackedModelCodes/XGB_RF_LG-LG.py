...
def conduct_optuna_study(X_train, y_train):
    ...
    def rf_objective(trial):
        try:
            ...
            n_estimators_rf = trial.suggest_int('n_estimators_rf', 50, 300)  # Number of trees
            max_depth_rf = trial.suggest_int('max_depth_rf', 3, 20)  # Maximum depth of the trees
            min_samples_split_rf = trial.suggest_int('min_samples_split_rf', 2, 10)  # Minimum samples to split a node
            min_samples_leaf_rf = trial.suggest_int('min_samples_leaf_rf', 1, 4)  # Minimum samples to be at a leaf node
...
def train_ensemble_model(best_params, X_train, y_train, model_path):
    ...
    This function trains a stacking ensemble model consisting of a RandomForest and an XGBoost model as base models,
    ...
    Args:
        best_params (dict): The best hyperparameters for each model.
            ...
            - 'rf': Hyperparameters for the RandomForest model.
                - 'n_estimators_rf' (int): Number of trees in the forest.
                - 'max_depth_rf' (int): Maximum depth of the tree.
    ...
    rf_model = RandomForestClassifier(
        n_estimators=best_params['rf']['n_estimators_rf'],
        max_depth=best_params['rf']['max_depth_rf'],
        random_state=42,
        class_weight='balanced',
        n_jobs=2
    )
    ...
    # Stacking ensemble with RandomForest and XGBoost as base models
...
def main():
    ...
    try:
        ...
        for fold_idx, (X_train, X_test, y_train, y_test) in enumerate(folds, start=1):
            ...
            model_path = os.path.join(base_dir, 'Test Models', 'Stacked Models (Optuna)', f'XGB_RF_LG_Fold_{fold_idx}.pkl')
            params_path = os.path.join(base_dir, 'Test Models', 'Stacked Models (Optuna)', f'XGB_RF_LG_Best_Params_Fold_{fold_idx}.json')
