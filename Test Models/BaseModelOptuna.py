...
def conduct_optuna_study(X_train, y_train, model_name):
    ...
    if model_name == 'XGBoost':
        ...
        best_params['min_samples_split'] = best_params.pop('min_samples_split_rf')
        best_params['min_samples_leaf'] = best_params.pop('min_samples_leaf_rf')
        ...
        def lgbm_objective(trial):
            ...
            learning_rate_lgbm = trial.suggest_float('learning_rate_lgbm', 0.01, 0.3)
...
def model_training(X_train, y_train, X_test, y_test, model, model_path, model_name):
    ...
    if model_name == 'XGBoost':
        ...
        model = SVC(**best_params, probability=True, class_weight='balanced', random_state=42)
    elif model_name == 'Logistic Regression':
        solver = 'saga' if best_params['penalty'] == 'l1' else 'lbfgs'
        model = LogisticRegression(**best_params, solver=solver, class_weight='balanced', random_state=42, max_iter=2000)
    elif model_name == 'Random Forest':
...
def main():
    ...
    try:
        ...
        for fold_idx, (X_train, X_test, y_train, y_test) in enumerate(folds, start=1):
            ...
            model_path = os.path.join(base_dir, 'Test Models', 'Saved Base Models (Optuna)')      
            param_path = os.path.join(base_dir, 'Test Models', 'Saved Base Models (Optuna)')
            models = {
            ...
            for model_name, model in models.items():
                logging.info(f"Training {model_name} model for Fold {fold_idx}...")
