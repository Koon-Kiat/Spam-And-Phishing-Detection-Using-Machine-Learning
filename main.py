...
def stratified_k_fold_split(df, n_splits=3, random_state=42, output_dir='Data Splitting'):
    ...
    for fold_idx, (train_index, test_index) in enumerate(skf.split(X, y), start=1):
        ...
        logging.info(f"Fold {
                     fold_idx} - y_train distribution: {y_train_counts}, Total: {len(y_train)}")
        logging.info(
...
def main():
    ...
    PreprocessedSpamAssassinFile = os.path.join(
        base_dir, 'Data Preprocessing', 'Preprocessed_SpamAssassin.csv')
    PreprocessedCEASFile = os.path.join(
        base_dir, 'Data Preprocessing', 'Preprocessed_CEAS_08.csv')
    ExtractedSpamAssassinEmailHeaderFile = os.path.join(
        base_dir, 'Feature Engineering', 'Spam_Assassin_Extracted_EmailHeader.csv')
    ExtractedCEASEmailHeaderFile = os.path.join(
        base_dir, 'Feature Engineering', 'CEAS_Extracted_EmailHeader.csv')
    MergedSpamAssassinFile = os.path.join(
        base_dir, 'Data Integration', 'Merged_SpamAssassin.csv')
    MergedCEASFile = os.path.join(
        base_dir, 'Data Integration', 'Merged_CEAS_08.csv')
    MergedDataFrame = os.path.join(
        base_dir, 'Data Integration', 'Merged_DataFrame.csv')
    CleanedDataFrame = os.path.join(
        base_dir, 'Data Cleaning', 'Cleaned_DataFrame.csv')
    CleanedCEASHeaders = os.path.join(
        base_dir, 'Data Cleaning', 'Cleaned_CEASHeaders.csv')
    MergedCleanedCEASHeaders = os.path.join(
        base_dir, 'Data Cleaning', 'Merged_Cleaned_CEAS_Headers.csv')
    MergedCleanedDataFrame = os.path.join(
        base_dir, 'Data Cleaning', 'Merged_Cleaned_DataFrame.csv')
    NoisyDataFrame = os.path.join(
        base_dir, 'Noise Injection', 'Noisy_DataFrame.csv')
    ...
    try:
        ...
        processor_spamassassin = DatasetProcessor(
            df_spamassassin, 'text', 'SpamAssassin', PreprocessedSpamAssassinFile)
        df_processed_spamassassin = processor_spamassassin.process_dataset()
        processor_ceas = DatasetProcessor(
            df_ceas, 'body', 'CEAS_08', PreprocessedCEASFile)
        ...
        spamassassin_headers_df = load_or_extract_headers(
            df_processed_spamassassin, ExtractedSpamAssassinEmailHeaderFile, EmailHeaderExtractor, 'Spam Assassin')
        logging.info(
        ...
        spamassassin_headers_df['urls'] = spamassassin_headers_df['texturls'].apply(
            count_urls)  # Convert text to number for URLs
        ...
        # Drop the 'mailto' column
        spamassassin_headers_df.drop(columns=['mailto'], inplace=True)
        # Drop the 'texturls' column
        spamassassin_headers_df.drop(columns=['texturls'], inplace=True)
        ...
        ceas_headers_df = load_or_extract_headers(
            df_processed_ceas, ExtractedCEASEmailHeaderFile, EmailHeaderExtractor, 'CEAS_08')
        ...
        df_cleaned_ceas_headers = load_or_save_emails(
            df_processed_ceas, CleanedCEASHeaders)
        ...
        if len(df_processed_spamassassin) == len(spamassassin_headers_df):
            merged_spamassassin_df = pd.merge(
                df_processed_spamassassin, spamassassin_headers_df, on='index', how='left')
            merged_spamassassin_df = merged_spamassassin_df.rename(
                columns={'text': 'body'})
            merged_spamassassin_df = merged_spamassassin_df[['sender', 'receiver', 'https_count', 'http_count',
            ...
            logging.info(f"Number of missing rows in Merged Spam Assassin Dataframe: {
                         len(missing_in_merged_df)}")
            logging.info(f'Total rows in Processed Spam Assassin Dataframe: {
                         len(df_processed_spamassassin)}')
            logging.info(f"Total rows in Merged Spam Assassin Dataframe: {
                         len(merged_spamassassin_df)}")
            ...
            logging.error(
                "The number of rows in Processed Spam Assassin and Spam Assassin Header do not match.")
            raise ValueError(
                "The number of rows in Processed Spam Assassin and Spam Assassin Header do not match.")
        ...
        if len(merged_spamassassin_df) != len(df_processed_spamassassin):
            logging.error(
                "The number of rows in the Merged Spam Assassin DataFrame DataFrame does not match Processed Spam Assassin.")
            raise ValueError(
                "The number of rows in the Merged Spam Assassin DataFrame does not match Processed Spam Assassin.")
        else:
            logging.info(
                "The number of rows in the Merged Spam Assassin DataFrame matches Processed Spam Assassin.")
            merged_spamassassin_df.to_csv(MergedSpamAssassinFile, index=False)
            logging.info(f"Merged Spam Assassin DataFrame successfully saved to {
                         MergedSpamAssassinFile}\n")
        ...
        if len(df_processed_spamassassin) == len(spamassassin_headers_df):
            merged_ceas_df = pd.merge(
                df_cleaned_ceas_headers_merge, ceas_headers_df, on='index', how='left')
            merged_ceas_df = merged_ceas_df[['sender', 'receiver', 'https_count', 'http_count',
                                             'blacklisted_keywords_count', 'short_urls', 'has_ip_address', 'urls', 'body', 'label', 'index']]
            missing_in_merged_df = merged_ceas_df[merged_ceas_df['index'].isnull(
            )]
            logging.info(f"Number of missing rows in Merged CEAS_08 Dataframe: {
                         len(missing_in_merged_df)}")
            logging.info(f'Total rows in Processed CEAS_08 Dataframe: {
                         len(df_cleaned_ceas_headers_merge)}')
            logging.info(f"Total rows in Merged CEAS_08 Dataframe: {
                         len(merged_ceas_df)}")
            ...
            logging.error(
                "The number of rows in Processed CEAS_08 and CEAS_08 Header do not match.")
            raise ValueError(
                "The number of rows in Processed CEAS_08 and CEAS_08 Header do not match.")
        ...
        if len(merged_ceas_df) != len(df_cleaned_ceas_headers_merge):
            logging.error(
                "The number of rows in the Merged CEAS_08 DataFrame DataFrame does not match Processed CEAS_08.")
            raise ValueError(
                "The number of rows in the Merged CEAS_08 DataFrame does not match Processed CEAS_08.")
        else:
            logging.info(
                f"The number of rows in the Merged CEAS_08 DataFrame matches Processed CEAS_08.")
            merged_ceas_df.to_csv(MergedCEASFile, index=False)
            logging.info(f"Merged CEAS_08 DataFrame successfully saved to {
                         MergedCEASFile}\n")
        ...
        combined_label_counts = combined_df['label'].value_counts(
        ).sort_index()
        ...
        logging.info(f"Merged DataFrame successfully saved to {
                     MergedDataFrame}")
        ...
        df_clean_body = load_or_clean_data(
            'Merged Dataframe', combined_df, 'body', CleanedDataFrame, data_cleaning)
        ...
        df_cleaned_combined = pd.concat([
            combined_df_reset[['sender', 'receiver', 'https_count', 'http_count', 'blacklisted_keywords_count',
                               # Select necessary columns from merged
        ...
        df_cleaned_combined = df_cleaned_combined[['sender', 'receiver', 'https_count', 'http_count',
                                                   'blacklisted_keywords_count', 'short_urls', 'has_ip_address', 'urls', 'cleaned_text', 'label']]
        ...
        noisy_df = generate_noisy_dataframe(
            df_cleaned_combined, NoisyDataFrame)
        ...
        for fold_idx, (X_train, X_test, y_train, y_test) in enumerate(folds, start=1):
            ...
            model_path = os.path.join(
                base_dir, 'Models & Parameters', f'Ensemble_Model_Fold_{fold_idx}.pkl')
            params_path = os.path.join(
                base_dir, 'Models & Parameters', f'Best_Parameter_Fold_{fold_idx}.json')
