...
def save_to_csv(dataset_name, df_processed, text_column, clean_file):
...
def extract_or_load_bert_features(dataset_name, df_clean, features_path):
...
def main():
    ...
    try:
        ...
        combined__percentage_df = pd.concat([df_processed_spamassassin, df_processed_ceas], ignore_index=True)
        ...
        header_extractor.save_to_csv(ExtractedSpamAssassinEmailHeaderFile)
        logging.info(
            "Email header extraction and saving from Spam Assassin completed.\n")


        # Data cleaning and saving the cleaned text data
        df_clean_ceas = save_to_csv("CEAS_08", df_processed_ceas, 'body', CleanedCeasFile)
        df_clean_spamassassin = save_to_csv("Spam Assassin", df_processed_spamassassin, 'text', CleanedSpamAssassinFile)
        ...
        ceas_bert_features = extract_or_load_bert_features("CEAS_08", df_clean_ceas, CEASExtractedFeatures)
        spamassassin_bert_features = extract_or_load_bert_features("Spam Assassin", df_clean_spamassassin, SpamAssassinExtractedFeatures)
        ...
        '''
        merged_spamassassin = pd.concat([spamassassin_headers_df.reset_index(drop=True), df_processed_spamassassin[['label']].reset_index(drop=True)], axis=1) # Merge df_processed_spamassassin with spamassassin_headers_df
        ...
        '''
        final_df = pd.concat([merged_spamassassin, df_processed_ceas], ignore_index=True) # Concatenate the DataFrames
        ...
        logging.info(f"Columns in merged dataset: {final_df.columns.tolist()}\n")
        '''
        ...
        combined_labels = combined__percentage_df['label'].reset_index(drop=True)
        ...
        if labels_match:
            ...
            log_label_percentages(combined__percentage_df, "Combined Percentage DataFrame")
        ...
        check_missing_values(final_df, "final_df")
        '''
        ...
        check_missing_values(preprocessed_df, "preprocessed_df")
        preprocessed_labels = preprocessed_df['label'].reset_index(drop=True)
        final_labels = final_df['label'].reset_index(drop=True)
        labels_match = preprocessed_labels.equals(final_labels)
        if labels_match:
            logging.info("The label columns in preprocessed_df and final_df match.")
        else:
            logging.error("The label columns in preprocessed_df and final_df do not match.")
            mismatched_labels = preprocessed_labels[preprocessed_labels != final_labels]
            logging.error(f"Mismatched labels:\n{mismatched_labels}") # Log the mismatched values for debugging
        ...
        # Verification of preprocessed_df, ceas_bert_features_df, and spamassassin_bert_features_df
        logging.info(f"Checking shapes:")
        logging.info(f"Shape of preprocessed_df: {preprocessed_df.shape}")
        logging.info(f"Shape of ceas_bert_features_df: {ceas_bert_features_df.shape}")
        logging.info(f"Shape of spamassassin_bert_features_df: {spamassassin_bert_features_df.shape}\n")
        logging.info("Checking missing values:")
        check_missing_values(preprocessed_df, "preprocessed_df")
        check_missing_values(ceas_bert_features_df, "ceas_bert_features_df")
        check_missing_values(spamassassin_bert_features_df, "spamassassin_bert_features_df")


        # Check the final features DataFrame
        final_features = pd.concat([preprocessed_df, spamassassin_bert_features_df, ceas_bert_features_df], axis=1)


        # Verification of the final features DataFrame
        check_missing_values(final_features, "final_features")
        logging.info(f"Number of duplicate rows in final_features: {final_features.duplicated().sum()}")
        logging.info(f"Number of samples in final_features: {final_features.shape[0]}")
        logging.info(f"Number of samples in preprocessed_df: {preprocessed_df.shape[0]}")
        ...
        if final_features.shape[0] != final_df.shape[0]:
            logging.info("Error: Row count mismatch between preprocessed data and BERT features.") # Ensure no misalignment between preprocessed_df and BERT feature dataframes
        if final_features.isnull().sum().sum() > 0:
            ...
            logging.info(f"Imputing missing values in final_features...")
            imputer = SimpleImputer(strategy='mean')
            final_features = pd.DataFrame(imputer.fit_transform(final_features), columns=final_features.columns)
            logging.info(f"Missing values imputed in final_features.\n")
