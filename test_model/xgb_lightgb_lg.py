...
def conduct_optuna_study(X_train, y_train):
    ...
    def lgb_objective(trial):
        try:
            ...
            n_estimators_lgb = trial.suggest_int('n_estimators_lgb', 50, 300)  # Number of boosting rounds
            num_leaves_lgb = trial.suggest_int('num_leaves_lgb', 31, 256)  # Maximum number of leaves in one tree
            max_depth_lgb = trial.suggest_int('max_depth_lgb', 3, 20)  # Maximum depth of the tree
            learning_rate_lgb = trial.suggest_float('learning_rate_lgb', 0.01, 0.3)  # Learning rate
            reg_alpha_lgb = trial.suggest_float('reg_alpha_lgb', 0.0, 10.0)  # L1 regularization
            reg_lambda_lgb = trial.suggest_float('reg_lambda_lgb', 0.0, 10.0)  # L2 regularization
            min_child_samples_lgb = trial.suggest_int('min_child_samples_lgb', 5, 20)  # Minimum samples to create a new leaf
            min_split_gain_lgb = trial.suggest_float('min_split_gain_lgb', 0.0, 1.0)  # Minimum gain to create a split
...
def main():
    ...
    try:
        ...
        for fold_idx, (X_train, X_test, y_train, y_test) in enumerate(folds, start=1):
            ...
            model_path = os.path.join(base_dir, 'Test Models', 'Stacked Models (Optuna)', f'XGB_LightGB_LG_Fold_{fold_idx}.pkl')
            params_path = os.path.join(base_dir, 'Test Models', 'Stacked Models (Optuna)', f'XGB_LightGB_LG_Best_Params_Fold_{fold_idx}.json')
